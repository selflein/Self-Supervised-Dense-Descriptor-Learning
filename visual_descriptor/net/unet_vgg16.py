...
class VGGUnet(nn.Module):
    ...
    def __init__(self, encoder, *, pretrained=False, out_channels=2, freeze=True):
        super().__init__()
...
def vgg13bn_unet(output_dim: int=2, pretrained: bool=False, freeze: bool = True):
    if not pretrained and freeze:
        raise ValueError
    return VGGUnet(vgg13_bn, pretrained=pretrained, out_channels=output_dim, freeze=freeze)


def vgg16bn_unet(output_dim: int=2, pretrained: bool=False, freeze: bool = True):
    if not pretrained and freeze:
        raise ValueError
    return VGGUnet(vgg16_bn, pretrained=pretrained, out_channels=output_dim, freeze=freeze)